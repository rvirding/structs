...
%%     $Id$
%%
-module(structs_transform).

%% An transformer of Erlang abstract syntax to handle Elixir structs.
...
-record(struct_data, {module,
		      anno,			%Struct anno
		      def_fields,		%Fields from default struct
		      map_fields,		%Fields as for default map
...
%% Collect the attributes and non-attributes.

collect_attributes([{attribute,_,_,_} = Attr|Fs]) ->
    [Attr | collect_attributes(Fs)];
collect_attributes([_Other|Fs]) ->
    collect_attributes(Fs);
collect_attributes([]) ->
    [].

collect_others([{attribute,_,_,_}|Fs]) ->
    collect_others(Fs);
collect_others([Other|Fs]) ->
    [Other | collect_others(Fs)];
collect_others([]) ->
    [].
...
form({attribute,Anno,defstruct,DefFields} = StructAttr) ->
    ...
    StructData = #struct_data{module = Mod,
			      anno = Anno,
    ...
    StructAttr;                                 %Save it in the module

form({attribute,Anno,module,Mod}) ->
    put(module_name, {name,Mod}),		%Save the current module name
...
pattern({record,Anno,?StrRec,
    ...
    {map,Anno,[StructModField | StructFields]};
...
check_struct(StrMod, Anno, Assocs) ->
    try
...
to_struct_map_fields({cons,_,{tuple,Fanno,[Key,Value]},Fs}, FieldType) ->
	...
	#struct_data{anno = Anno,
		...
		[{call,Anno,{remote,Anno,{atom,Anno,lists},{atom,Anno,foldl}},
		  [{'fun', Anno,
		    {clauses,
		     [{clause, Anno,
		       [{tuple, Anno, [{var,Anno,'Key'},{var,Anno,'Val'}]},
			{var,Anno,'Map'}],
		       [],
		       [{call, Anno,
			 {remote,Anno,{atom,Anno,maps},{atom,Anno,update}},
			 [{var,Anno,'Key'},
			  {var,Anno,'Val'},
			  {var,Anno,'Map'}]}]}]}},
		   DefStruct,
